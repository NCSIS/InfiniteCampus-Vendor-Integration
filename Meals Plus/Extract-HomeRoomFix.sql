/*
  ...
  11/5/2025    Remove all special characters and numbers from the homeroom field.
...
LatestEnrollment AS (
    SELECT 
        ...
        stu.endDate,
        ROW_NUMBER() OVER (
            PARTITION BY stu.personID 
            ORDER BY 
                CASE WHEN stu.endDate IS NULL THEN 0 ELSE 1 END,  -- Prioritize NULL dates first
                stu.endDate DESC                                -- Then order by date descending
        ) AS rowNumber
...
CASE
    WHEN CHARINDEX('(', s.homeroomTeacher) > 0 THEN
        LTRIM(RTRIM(
            REPLACE(
                TRANSLATE(
                    LEFT(s.homeroomTeacher, CHARINDEX('(', s.homeroomTeacher) - 1),
                    '0123456789!@#$%^&*()-_=+[]{};:'',.<>/?\|`~"',
                    REPLICATE(' ', LEN('0123456789!@#$%^&*()-_=+[]{};:'',.<>/?\|`~"'))
                ),
                '  ', ' '
            )
        ))
...
FROM v_AdHocStudent s
	...
	LEFT JOIN ContactSelf cs ON s.personID = cs.personID AND cs.rowNumber = 1
